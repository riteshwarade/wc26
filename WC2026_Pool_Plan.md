...
| Date | Change |
| Jun 19, 2026 | **Fix bracket layout on landscape mobile.** Desktop 5-column bracket (`.bk-outer`) was rendering on landscape phones (667–896px wide) because the landscape media query was missing the tab-view switch. Added `.bk-outer { display: none }` + `.bk-mobile-tabs { display: block }` to the existing landscape block in Leaderboard Swiftly (propagated to FandF via `make_fandf.py`). Added a new landscape block with the same two rules to KO Picks page, which had no landscape query at all. |
