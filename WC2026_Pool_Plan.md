...
| Date | Change |
| Jul 16, 2026 | **Update `WC2026_Bracket_Sharing.html` with confirmed semifinal results.** Spain beat France 2-0 (M101), Argentina beat England 2-1 (M102) — `SF_MATCHES` now carries real scores + winner/loser styling, matching the QF cards. `FINAL_MATCH` and `THIRD_MATCH` updated from `W101`/`W102`/`L101`/`L102` placeholders to the real matchups (Spain v Argentina; France v England) — shown as plain undecided cards since neither has been played yet. Podium stays all-TBD. Snapshot note text updated. Verified via a Node `vm` harness stubbing `document` so `bracket.js`'s `matchCard()`/`buildPodiumHtml()` run headless and return the expected HTML (winner/loser classes, scores, round labels) — confirmed before handing off for push. Manual update per the standing decision to keep this page hand-maintained rather than auto-fetching. |
| Jul 16, 2026 | **Reword the cascaded-square tooltip.** "⚡ Void — team eliminated earlier" → "⚡ Void — this pick can no longer happen". The old copy was inaccurate for the M103-SF-winner cascade case fixed earlier today (that team wasn't eliminated, it advanced to the Final instead) — the new wording is accurate for every cascade reason without branching on match number. `leaderboard.js`, `_showSqTooltip()`. Visualized two options first (generic rewrite vs. a match-103-specific message); went with the generic one to avoid extra branching logic. |
| Jul 16, 2026 | **Fix inflated `maxPts` for M103 (3rd place) picks of an SF winner.** `evalKoPicks` in `scoring.js` only cascaded a pending KO pick if the picked team was *eliminated* earlier than needed. M103 is the one match where eligibility runs the other way — it's fed by the two SF *losers*, so a pick of either SF *winner* is just as dead once both SF results are in, but the elimination check never caught it (winners are never "eliminated"). Surfaced when a user noticed their live `maxPts` (254) was 12 pts higher than their `totalPts` (242) despite having no actual remaining upside — their M103 pick was Spain, an SF winner. Fixed by also checking the pick against `getKoTeams(103, bracketData, koResults)` (the two real M103 participants) once both SF results are known, cascading anything that isn't one of them. At the time of the fix, roughly half of both pools (15/33 Swiftly, 38/63 FandF) had picked an eventual SF winner for 3rd place and were affected. Added a dedicated test (`test_leaderboard.js`) covering the pending-not-yet-cascaded window: SF winner pick → cascaded + excluded from maxPts, real SF loser pick → still correctly pending + counted toward maxPts. Pure scoring-engine fix — `leaderboard.js` already renders `cascaded` correctly, no UI changes needed. |
| Jul 12, 2026 | **Fix broken div nesting in the "Tiebreakers" tooltip addition (same-day regression).** The previous commit's markup had an extra `</div>` that closed `.scoring-tooltip-box` right after the "Final" row, making `.stt-tb-title`/`.stt-tb-list` render as siblings of the box instead of children — they showed up unstyled, full-width, and always-visible (not gated by hover) directly under the "Scoring" pill, with the actual bordered tooltip box floating separately below containing just the points table. Caught from a live screenshot after pushing. Fixed by moving the closing `</div>` to after the tiebreaker markup. Also fixed a related latent bug: `.stt-row:last-of-type { border-bottom: none }` matches by tag position, not class, so once `.stt-tb-title` (also a `<div>`) became the true last div child, the Final row's border-bottom would no longer be suppressed, producing a doubled divider. Replaced with `.stt-row + .stt-row { border-top: ... }` (border between consecutive rows only) so it's correct regardless of what comes after the last row. Verified by live-patching the already-deployed page via Claude in Chrome (rewriting `#scoring-info-anchor`'s HTML + injecting the corrected CSS at runtime, then screenshotting) before pushing the real fix — confirmed the tiebreaker list now renders inside the same box, below the table, no double line. |
| Jul 12, 2026 | **Add a "Tiebreakers" list to the scoring tooltip on KO standings.** `_scoringTooltipHtml` in `leaderboard.js` now appends `<div class="stt-tb-title">Tiebreakers</div>` + `<ul class="stt-tb-list">` (Correct champion pick / Total correct picks / Still tied? Stay tied.) below the existing points-per-round table, so participants can see the post-Final tiebreak order without leaving the standings page. New CSS (`.stt-tb-title`, `.stt-tb-list`) added to `WC2026_Pool_Leaderboard_Swiftly.html`, styled to match the existing `.stt-row` text (same size/weight/color) rather than the bolder unused `.stt-title` class, so it reads as a continuation of the list. Visualized several iterations first (font size, bullets vs prose, exact wording, heading text/style) before implementing. `WC2026_Pool_Leaderboard_FandF.html` regenerated via `make_fandf.py`. Pure markup/CSS addition — not exercised by the test suites, but ran all 4 CI suites + 152 e2e + 58 ko_picks + leaderboard tests to confirm nothing else broke (same 2 pre-existing KO-only-participants failures as before). |
| Jul 12, 2026 | **Make the pool-standings tiebreaker chain two-phase; remove groupPts as a tiebreak; make ties permanent.** `computeCombinedStandings`'s final sort (`scoring.js`) now branches on `!!koResults[104]` (has the Final been played?). Mid-tournament: totalPts desc, then name asc — name only orders which row prints first among a tie, it no longer implies a real ranking. Post-Final: totalPts desc → correctChampion desc → totalCorrect desc, then name asc. In both phases, anyone still equal after those criteria stays tied — nothing forces a full ordering anymore. `groupPts` (added as tiebreak criterion #4 earlier today, see next row) is removed from the sort entirely and is now display-only. Updated `_rankMap` in `leaderboard.js`'s `renderKoStandings` to match: the `#` badge ties on totalPts alone pre-Final, and on totalPts+correctChampion+totalCorrect post-Final, so displayed ranks reflect genuine ties rather than the old totalPts-only grouping. A tied group can now legitimately show more than 3 rows as `top3` if they're tied for a podium spot — expected, not a bug. Replaced the "most group points" test in `test_leaderboard.js` with two ties-stay-tied tests (post-Final: Eve/Frank tied through totalCorrect, differing only on groupPts, confirmed still tied; mid-tournament: Alice/Mike/Zara tied only on totalPts, differing on groupPts and totalCorrect, confirmed still tied). Updated `test_e2e.js`'s sort-order invariant to drop the groupPts clause. All 4 CI suites + 152 e2e + 58 ko_picks + 86/88 leaderboard tests pass (same 2 pre-existing, unrelated KO-only-participants failures as before, not part of CI). |
| Jul 12, 2026 | **Add a 4th pool-standings tiebreaker: most group points.** `computeCombinedStandings`'s final sort (`scoring.js`) now goes totalPts desc → correctChampion desc → totalCorrect desc → **groupPts desc (new)** → name asc. Previously the chain fell straight from totalCorrect to alphabetical name; now someone with a stronger group-stage showing ranks above an otherwise-tied participant before it comes down to name. Updated `test_e2e.js`'s sort-order invariant check to validate the new 5-level chain, and added a dedicated tiebreak test to `test_leaderboard.js` (Eve vs Frank, tied on the first three criteria, differing only on groupPts). All 4 CI suites + the new/updated leaderboard tests pass (2 pre-existing, unrelated `test_leaderboard.js` failures in the KO-only-participants section predate this change and aren't part of CI). |
| Jul 12, 2026 | **Give losing team rows a visible grey outline (live bracket, KO picks page, and the sharing snapshot).** `.bk-team.l` border-color `transparent` → `#c4c4c4` in `WC2026_Pool_Leaderboard_Swiftly.html` and `WC2026_Pool_Knockout_Picks.html` (the latter applies to whichever team a user didn't pick, via `ko_picks.js`). Mobile equivalent `.bk-mob-team.bk-mob-los` got the same fix. `WC2026_Pool_Leaderboard_FandF.html` regenerated via `make_fandf.py`. Hover state unaffected — `.l:hover` already falls through to the generic `:hover` rule. Tried first on `WC2026_Bracket_Sharing.html` (visualized a couple of options — a light blue border around the whole card was tried and rejected; the grey loser-row outline was kept), then ported to the shared product styles once confirmed. All 152 e2e + 58 ko_picks + bracket tests still pass (CSS-only change, not exercised by those suites but confirms nothing else broke). |
| Jul 12, 2026 | **Double horizontal round spacing in `WC2026_Bracket_Sharing.html`.** `.bk-col` padding `0 10px` → `0 20px` — gap between adjacent round columns (QF↔SF, SF↔Final) goes from 20px to 40px. Scoped to this snapshot page only, not the live leaderboard bracket. |
| Jul 12, 2026 | **Add 3rd place card + podium to `WC2026_Bracket_Sharing.html`.** M103 (`L101`/`L102` placeholders, same `getKoTeams()` convention as the live site since the SF losers aren't known yet) and `buildPodiumHtml(null, null, null)` (all three rows TBD) now render inside the `fin` float alongside M104, mirroring the real bracket's `floatCol('fin', [104], thirdPlaceHtml + podiumSection)` structure. `.bk-col-hdr` for that column updated to "🏆 Final (and 3rd place)" to match. Positioning is adapted from `positionAndConnectBracket()`'s dedicated M103/podium block, but pinned to different anchors per explicit instruction: M103 aligns with M100's team-dividing line (real bracket uses M99); podium's top aligns with M97's top (real bracket uses M89, the first card of the first float tier after the anchor — M97 is this page's structural analog since QF is the anchor here instead of R32). `.bk-podium*` CSS copied verbatim from the Swiftly HTML. Verified via the same Node harness (mocked `document`, direct `matchCard()`/`buildPodiumHtml()` calls) — confirmed `3rd · M103 · ...` header and correct TBD podium rows. |
| Jul 12, 2026 | **Update `WC2026_Bracket_Sharing.html` with QF results.** All four quarterfinals confirmed: France d. Morocco 2–0, Spain d. Belgium 2–1, England d. Norway 2–1, Argentina d. Switzerland 3–1. QF cards now render with real winner/loser styling + scores (`matchCard(num, home, away, '', homeScore, awayScore, homeCls, awayCls)`), matching the live results bracket's look. SF matchups updated to the real teams (France v Spain for M101, England v Argentina for M102) — shown as plain undecided cards since the semifinals haven't been played. Final (M104) stays as the `W101`/`W102` TBD placeholder. Snapshot note text updated to reflect "confirmed through the quarterfinals." Manual update per the earlier decision to keep this page hand-maintained rather than auto-fetching. |
| Jul 9, 2026 | **Show points won in the square tooltip.** `_showSqTooltip()` (`leaderboard.js`) now appends a `+N pts` line under the status line whenever a pick actually scored (`correct`/`correct-upset`) — hidden for wrong/pending/cascaded/empty. No new data plumbing: derives the value from what's already on the square (`data-match`, `data-status`) — KO matches (73-104) look up the existing round-scaled `KO_POINTS` table from `scoring.js` (4/8/12/16/12/24 by round), group matches (1-72) are a flat `2`. One change point covers both group and KO squares, desktop and mobile, since they share `_showSqTooltip()`. Added `.tt-points` CSS (blue, bold, matches `.tt-status.correct`) to the Swiftly HTML; FandF regenerated via `make_fandf.py`. All 152 e2e + 58 ko_picks tests still pass (tooltip UI isn't covered by those suites, but the underlying `KO_POINTS` values they exercise are the same table this reads from). |
| Jul 9, 2026 | **Rename `WC2026_Bracket_QF_to_Final.html` → `WC2026_Bracket_Sharing.html`.** No content change — shorter, cleaner name for the file that gets shared/screenshotted with pool participants. |
| Jul 8, 2026 | **Replace the bracket PNG with `WC2026_Bracket_Sharing.html` (then named `WC2026_Bracket_QF_to_Final.html`) — same shareable QF/SF/Final snapshot, rendered as a real web page instead of a rasterized PNG.** The cairosvg pipeline behind the PNG couldn't render color-emoji-font flags natively (needed a twemoji-vendoring workaround) and required manual re-rasterization after every change. The HTML version sidesteps both: it loads the actual `bracket.js` via `<script src="bracket.js">` and calls its real `matchCard()`/`bkTeamRow()` functions directly, so card markup, flags, FIFA ranks, and header format (`{round} · M{num} · {date}`) are byte-identical to the live leaderboard bracket rather than a redrawn approximation. `.bk-*` CSS copied verbatim from `WC2026_Pool_Leaderboard_Swiftly.html`. Positioning/connector-drawing is a small inline script adapted from `positionAndConnectBracket()`/`drawBracketConnectors()`, scoped to just the QF→SF→Final levels (the real functions require R32 DOM elements this page doesn't have). QF data: France v Morocco, Spain v Belgium, Norway v England, Argentina v Switzerland — all four now fully confirmed as of this session (see M96 note below). SF/Final show real `W{num}` TBD placeholders. Verified card markup via a Node harness that loads `bracket.js` and calls `matchCard()` directly (mocked `document`), confirming exact HTML/text/classes for both confirmed and TBD cards; full visual verification deferred to the user opening the page in a browser (file:// bracket rendering can't be screenshotted from this environment). **Side finding:** while confirming M100 (Argentina's QF opponent), discovered `results/knockout_results.csv` was momentarily missing the M96 result (Switzerland d. Colombia on penalties, 0-0 4-3) even though ESPN had it as final — the site's auto-update pipeline hadn't picked it up yet as of the last local commit. Resolved once the user pulled latest from `origin/main`, which already had the M96 row. The old PNG (`WC2026_Bracket_QF_to_Final.png`) is left in the repo as a stale, superseded snapshot rather than deleted. |
| Jul 8, 2026 | **Add `WC2026_Bracket_QF_to_Final.png` — shareable branded bracket graphic.** Static PNG showing QF, SF, and Final only (R32/R16/3rd place excluded per request). QF matchups filled in with actual confirmed R16 winners (France v Morocco, Spain v Belgium, Norway v England, Argentina v Switzerland); SF/Final shown as `W101`/`W102`-style TBD placeholders. First pass used a simplified custom card style; revised on request to mirror the actual leaderboard bracket's visual language exactly — same team-row colors/borders, same `#B5D4F4` connector line color, same `{round} · M{num} · {date}` header format, same TBD/em-dash convention, real flags (fetched as twemoji SVGs via jsdelivr, since `cairosvg` can't render color-emoji-font flags directly — rasterized to PNG and embedded) with FIFA ranks alongside each team name. Built via SVG template → `cairosvg` rasterization; uses the Swiftly logo from the `swiftly-brand-guidelines` skill's bundled assets (never regenerated/traced) and Poppins SemiBold + Inter fetched from Google Fonts at build time. Point-in-time snapshot, not auto-updating — regenerate manually as the bracket progresses. |
| Jul 4, 2026 | **Fix KO correctness pill denominator to count all KO players, not just resolvable picks.** `buildKoCounts` previously excluded cascaded picks from `total`, inflating the percentage — e.g. M90 showed `13/16` (81%, denominator = correct+wrong only) when the honest number is `13/33` (39%, denominator = all 33 KO participants), since 17 of those 33 had a cascaded pick (Netherlands/South Africa, eliminated in R32) rather than a valid correct/wrong outcome for that match. Added `koParticipant` flag in `scoring.js`'s `computeCombinedStandings` (both the group-participant branch and the KO-only-stragglers branch) — `true` iff the participant's raw KO picks object is non-empty — so the new denominator (`combined.filter(p => p.koParticipant).length`) correctly excludes people who never submitted KO picks at all (3 such people currently in Swiftly, 15 in FandF) without excluding people who played KO but had this particular pick cascade. Verified against live data: M89 → 31/33, M90 → 13/33. Left the "Knockout Standings" header player-count badge as total pool size (unchanged, by explicit decision) — it will legitimately read higher than any individual pill's denominator. All 5 test suites still pass. |
| Jul 4, 2026 | **Fix two `KO_SCHEDULE` kickoff-time bugs found via full ESPN audit.** M89 (Paraguay v France) and M90 (Canada v Morocco) had their kickoff times swapped at data entry (M89 listed 17:00Z/actually 21:00Z, M90 listed 21:00Z/actually 17:00Z) — surfaced as a mobile "Recent" squares column showing M90 as more recent than M89 when M89 actually kicked off and finished later. M79 (Mexico v Ecuador) was stale at the originally-announced 01:00Z; actual kickoff was 02:00Z after a 1-hour thunderstorm delay in Mexico City. Verified by cross-checking all 32 KO slots against the ESPN scoreboard API (same endpoint used for results) — the other 29 slots matched exactly. Documented `KO_SCHEDULE`'s canonical source (ESPN, cross-checked against Wikipedia for topology) in `CLAUDE.md` since it previously had none. `bracket.js` is shared, no `make_fandf.py` regen needed. All 5 test suites still pass. |
| Jul 4, 2026 | **Fix cascaded KO picks showing "live · wrong so far" during a live match.** A pick whose team was already eliminated in an earlier round (cascaded) was having its status overwritten by the live-overlay logic in `leaderboard.js` whenever the current match was live — it compared the cascaded pick's (eliminated) team against the live score of a match it isn't even part of, always resolving to `live-wrong`. Fixed with a one-line guard (`&& pr.status !== 'cascaded'`) so cascaded picks are skipped by the live overlay and keep showing the correct "⚡ Void — team eliminated earlier" tooltip. `leaderboard.js` is shared (not duplicated per-pool), so no `make_fandf.py` regen needed. All 152 e2e invariants + 58 ko_picks tests still pass. |
...
| Jun 29, 2026 | **Fix: KO standings disappear on sort click.** `renderKoStandings` replaces `koStandingsSection.innerHTML`, destroying the `.section-label` and `.section-body` DOM nodes that `_revealSections()` had previously set to `visibility:visible`. New nodes inherited `visibility:hidden` from CSS. Fixed by inlining `style="visibility:visible"` on both elements in the template. FandF regenerated. |
| Jun 29, 2026 | **Sortable columns in KO standings.** Grp, KO, Total, and Max column headers are now clickable to sort. Module-level `_koSortCol` (default `'total'`) and `_koSortDir` (default `'desc'`) persist across live re-renders. Click once = sort desc; click again = flip to asc; click different column = reset to desc. Rank (`#`) frozen to Total-desc order via `_rankMap` (computed from unsorted `combinedStandings`). Sort operates on a `[...combinedStandings]` copy — `_lastKoCombined` unchanged. Tiebreaker: Total desc → name `localeCompare`. Click listeners re-attached after each `innerHTML =` assignment. Also renamed Podium header from "PODIUM" to "1·2·3". FandF regenerated, all tests pass. |
| Jun 28, 2026 | **Fix KO standings landscape scroll.** Landscape media query only applied `overflow-x: auto` to `#standings-body .card` (group). Extended selector to also cover `#koStandingsSection .card` so KO standings scroll horizontally on landscape phones. FandF regenerated. |
| Jun 28, 2026 | **Podium column: replace Champion with 3-flag podium (🥇🥈🥉).** Replaced single-flag Champion column with a Podium column showing three flags per user: M104 pick (gold), implied runner-up derived from SF picks M101/M102 (silver), M103 pick (bronze). Faded flags (`opacity: 0.2`) when team is eliminated. Header reads "PODIUM" on both desktop and mobile — no dual-span needed. Old `.champ-*` CSS removed; replaced with `.podium-flags` + `.podium-fl`. FandF regenerated. |
| Jun 28, 2026 | **Mobile bracket: separate flag into own flex child (all 3 variants).** Added `mobTeamHtml(name)` to `bracket.js` — renders flag in `.bk-mob-fl` (flex-shrink: 0) and name+rank in `.bk-mob-tn` (flex: 1, overflow ellipsis), mirroring desktop `bkTeamRow`. Fixes row height inconsistency between known-team rows (flag emoji inflated line-box) and TBD rows. Updated all 3 mobile card builders: `mobMatchCard` (V1), `mobPickCard` (V2), `koMobMatchCard` (V3). Replaced `.bk-mob-team-name` CSS with `.bk-mob-fl` + `.bk-mob-tn` in both HTML files. FandF regenerated. |
| Jun 28, 2026 | **Cascaded squares: transparent red outline instead of solid red.** `.sq-cascaded` changed from `background: var(--swiftly-red)` to `background: transparent; border: 1.5px solid var(--swiftly-red)`. Distinguishes voided-future picks (game not played yet, team eliminated) from confirmed-wrong (solid red). FandF regenerated. |
| Jun 28, 2026 | **KO bracket live card improvements: tied pulsing, live pill, strip date/time.** (1) Tied matches: both team rows now get `.live-tied` (pulse animation, default styling) instead of only the label pulsing. (2) Live correctness pill: computed inline from `_lastKoCombined` — shows who has the current leader picked; hidden when tied (no leader). Minute shown first, pill second (swapped). (3) Date/time stripped from bracket card header once match is live or final — shows only `R32 · M73`. `matchCard()` gets new `suppressDate=false` 13th param; `mkCard` and `koMobMatchCard` pass `true` when `winner` or `lm` is set. Mobile card also gets tied class and swapped order. FandF regenerated. |
| Jun 28, 2026 | **Add `live-draw` (amber) square state for tied KO matches.** KO picks are team names, not Draw — when the score is tied the outcome is unknown. Added `.sq-live-draw` CSS (`--swiftly-orange`, same pulse animation) and `live-draw` sqStatus in the KO live overlay block. Tooltip shows `🔴 Live · tied`. Group stage unaffected (Draw is a valid pick there so tied → `live-correct` if they picked Draw). FandF regenerated. |
