...
| Date | Change |
| Jun 29, 2026 | **Sortable columns in KO standings.** Grp, KO, Total, and Max column headers are now clickable to sort. Module-level `_koSortCol` (default `'total'`) and `_koSortDir` (default `'desc'`) persist across live re-renders. Click once = sort desc; click again = flip to asc; click different column = reset to desc. Rank (`#`) frozen to Total-desc order via `_rankMap` (computed from unsorted `combinedStandings`). Sort operates on a `[...combinedStandings]` copy — `_lastKoCombined` unchanged. Tiebreaker: Total desc → name `localeCompare`. `_arrow(col)` emits active (Swiftly Blue ▼/▲) or inactive (neutral ⇅) indicator. Click listeners re-attached after each `innerHTML =` assignment. CSS: `cursor: pointer`, `user-select: none`, arrow colors. Also renamed Podium header from "PODIUM" to "1·2·3". FandF regenerated, all tests pass. |
